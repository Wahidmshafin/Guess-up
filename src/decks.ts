export type Deck = {
  id: string
  name: string
  emoji: string
  color: string
  description?: string
  words: string[]
  custom?: boolean
}

export const ALL_DECKS: Deck[] = [
  {
    id: "starter",
    name: "Combined Pack",
    emoji: "\ud83c\udf89",
    color: "#F2B705",
    description: "The ultimate party mega-mix with the best cards from all categories!",
    words: [
      "Elephant",
      "Pizza",
      "Guitar",
      "Rainbow",
      "Astronaut",
      "Bicycle",
      "Volcano",
      "Penguin",
      "Firefighter",
      "Rollercoaster",
      "Helicopter",
      "Telescope",
      "Submarine",
      "Campfire",
      "Fireworks",
      "Pyramid",
      "Kangaroo",
      "Cupcake",
      "Sunglasses",
      "Snowman",
      "Beder Meye Josna",
      "Hawa",
      "Toofan",
      "Aynabaji",
      "Monpura",
      "Dipu Number Two",
      "Debi",
      "Dhaka Attack",
      "Poran",
      "Shurongo",
      "Jibon Theke Neya",
      "Kabhi Khushi Kabhie Gham",
      "Keyamot Theke Keyamot",
      "Den Mohor",
      "Lal Shalu",
      "Dilwale Dulhania Le Jayenge",
      "Sholay",
      "3 Idiots",
      "Dangal",
      "Kuch Kuch Hota Hai",
      "Lagaan",
      "Kal Ho Naa Ho",
      "Zindagi Na Milegi Dobara",
      "Taare Zameen Par",
      "PK",
      "Chennai Express",
      "Om Shanti Om",
      "KGF",
      "Baahubali",
      "Chak De! India",
      "Don (Shah Rukh Khan)",
      "Krrish",
      "Dhoom",
      "Jab We Met",
      "My Name is Khan",
      "Munna Bhai M.B.B.S.",
      "Lage Raho Munna Bhai",
      "Hera Pheri",
      "Phir Hera Pheri",
      "Golmaal",
      "Welcome",
      "Yeh Jawaani Hai Deewani",
      "Stree",
      "Housefull",
      "Jawan",
      "Pathaan",
      "Animal",
      "12th Fail",
      "Student of the year",
      "Kabir Singh",
      "Sacred Games",
      "Mirzapur",
      "Scam 1992: The Harshad Mehta Story",
      "Harry Potter",
      "Titanic",
      "The Avengers",
      "Inception",
      "The Dark Knight",
      "Interstellar",
      "Avatar",
      "The Lord of the Rings",
      "Jurassic Park",
      "Star Wars",
      "The Matrix",
      "Shin-chan",
      "Oppenheimer",
      "Barbie",
      "Spider-Man",
      "Doremon",
      "The Lion King",
      "Forrest Gump",
      "Superman",
      "Pirates of the Caribbean",
      "Batman",
      "Home Alone",
      "Iron Man",
      "Pokemon",
      "The Prestige",
      "Fast & Furious",
      "John Wick",
      "Dune",
      "Deadpool",
      "Breaking Bad",
      "Game of Thrones",
      "Squid Game",
      "Money Heist",
      "Friends",
      "The Office",
      "Sherlock",
      "The Dark Night Rises",
      "Brushing your teeth",
      "Walking on a rope",
      "Opening a bottle",
      "Stuck in heavy traffic",
      "Surfing a huge wave",
      "Walking on the Moon",
      "Riding a crazy rollercoaster",
      "Sneezing",
      "Eating a pizza slice",
      "Eating extremely spicy chili",
      "Milking a cow",
      "Playing a guitar",
      "Solving a puzzle",
      "Skiing down a steep mountain",
      "Getting caught in torrential rain",
      "Taking a flattering selfie",
      "Riding a bike",
      "Running away from a bee",
      "Singing loudly in the shower",
      "Zombie walking",
      "Juggling flaming torches",
      "Sumo wrestling match",
      "Changing a baby diaper",
      "Walking against strong wind",
      "Dancing ballet",
      "Winning the lottery jackpot",
      "Opening a jar",
      "Karate kicking",
      "Escaping quicksand",
      "Bus conductor",
      "Tasting sour lemon",
      "Arm wrestling",
      "Walking on hot weather",
      "Bowling a strike",
      "Hypnotizing someone",
      "Putting on makeup",
      "Crying",
      "Looking for someone",
      "Figting with someone",
      "Swimming in a pool",
      "Taking a Bath",
      "Smoking",
      "Sleeping",
      "Lion",
      "African Elephant",
      "Chimpanzee",
      "Bottlenose Dolphin",
      "Giraffe",
      "Sloth",
      "Rat",
      "Giant Octopus",
      "Cheetah",
      "Giant Panda",
      "Koala",
      "Peacock",
      "Crocodile",
      "Silverback Gorilla",
      "Zebra",
      "Great Horned Owl",
      "Human",
      "Seahorse",
      "Polar Bear",
      "Platypus",
      "Buffalo",
      "Shark",
      "Bald Eagle",
      "Parrot",
      "Turtle",
      "Cuckoos",
      "Wolf",
      "Hippopotamus",
      "Komodo Dragon",
      "Hummingbird",
      "Snake",
      "Grizzly Bear",
      "Ostrich",
      "Blue Whale",
      "Jellyfish",
      "Red Fox",
      "Monkey",
      "Tiger",
      "Crow",
      "Camel",
      "Squid",
      "Orca",
      "Mongoose",
      "Duck",
      "Goose",
      "Shakib Al Hasan",
      "Mashrafe Bin Mortaza",
      "Tamim Iqbal",
      "Mushfiqur Rahim",
      "Mustafizur Rahman",
      "Humayun Ahmed",
      "James (Nagar Baul)",
      "Ayub Bachchu",
      "Runa Laila",
      "Sabina Yasmin",
      "Chanchal Chowdhury",
      "Mosharraf Karim",
      "Shakib Khan",
      "Salman Shah",
      "Jaya Ahsan",
      "Tahsan Khan",
      "Asif Akbar",
      "Habibul Bashar",
      "Sachin Tendulkar",
      "Virat Kohli",
      "MS Dhoni",
      "Rohit Sharma",
      "Shah Rukh Khan",
      "Amitabh Bachchan",
      "Salman Khan",
      "Aamir Khan",
      "Arijit Singh",
      "Lata Mangeshkar",
      "A.R. Rahman",
      "Shreya Ghoshal",
      "Kishore Kumar",
      "Deepika Padukone",
      "Priyanka Chopra",
      "Rajinikanth",
      "Hrithik Roshan",
      "Kapil Dev",
      "Sourav Ganguly",
      "Sania Mirza",
      "Alia Bhatt",
      "Shahid Afridi",
      "Atif Aslam",
      "Babar Azam",
      "Wasim Akram",
      "Imran Khan",
      "Shoaib Akhtar",
      "Rahat Fateh Ali Khan",
      "Cristiano Ronaldo",
      "Lionel Messi",
      "Taylor Swift",
      "Michael Jackson",
      "Tom Cruise",
      "Leonardo DiCaprio",
      "Dwayne \"The Rock\" Johnson",
      "Elon Musk",
      "MrBeast",
      "Jackie Chan",
      "Eminem",
      "Justin Bieber",
      "Selena Gomez",
      "Beyonc\u00e9",
      "Rihanna",
      "Keanu Reeves",
      "Brad Pitt",
      "David Beckham",
      "Gordon Ramsay",
      "Morgan Freeman",
      "Adele",
      "Ed Sheeran",
      "Bruno Mars",
      "Billie Eilish",
      "John Cena",
      "Undertaker",
      "Donald Trump",
      "Barack Obama",
      "William Shakespeare",
      "Albert Einstein",
      "Isaac Newton",
      "Charlie Chaplin",
      "Adolf Hitler",
      "Muhammad Ali",
      "Pizza Margherita",
      "Sushi & Sashimi",
      "Chocolate Ice Cream",
      "Crispy Tacos",
      "French Fries",
      "Pancakes with Maple Syrup",
      "Boba Bubble Tea",
      "Double Cheeseburger",
      "Hot Espresso",
      "Spaghetti Bolognese",
      "Guacamole and Tortilla Chips",
      "Fluffy Cotton Candy",
      "Butter Croissant",
      "Japanese Ramen",
      "Chicago Hot Dog",
      "Sweet Watermelon",
      "Spicy Burrito",
      "Glazed Donuts",
      "Avocado Toast",
      "Barbecue Ribs",
      "Buttery Popcorn",
      "Cinnamon Churros",
      "Belgian Waffles",
      "Chicken Nuggets",
      "Lasagna",
      "Fish and Chips",
      "Chocolate Milkshake",
      "Pad Thai",
      "Fresh Dim Sum",
      "Cheese Fondue",
      "Biryani",
      "Falafel",
      "Samosa",
      "Tiramisu",
      "Smoothie Bowl",
      "Eiffel Tower (Paris)",
      "Statue of Liberty (New York)",
      "Great Wall of China",
      "Taj Mahal (India)",
      "Great Pyramids of Giza",
      "Colosseum (Rome)",
      "London",
      "Mount Everest",
      "Cox's Bazar",
      "Sundarban",
      "Mount Fuji (Japan)",
      "Sreemongol",
      "Amazon River",
      "Neil River",
      "Burj Khalifa (Dubai)",
      "Sahara Desert",
      "Kashmir",
      "Panjab",
      "Hollywood Sign (Los Angeles)",
      "Petra (Jordan)",
      "Bandarban",
      "Bagdad",
      "Tehran",
      "Mecca",
      "Madina",
      "Riyad",
      "Google",
      "YouTube",
      "Facebook (Meta)",
      "Instagram",
      "TikTok",
      "WhatsApp",
      "Alibaba",
      "Amazon",
      "Apple (iPhone)",
      "Microsoft",
      "Netflix",
      "Spotify",
      "Twitter (X)",
      "ChatGPT (OpenAI)",
      "Uber",
      "Airbnb",
      "LinkedIn",
      "Reddit",
      "PlayStation (Sony)",
      "Xbox",
      "Nintendo",
      "Intel",
      "Nvidia",
      "Tesla",
      "Samsung",
      "Xiaomi",
      "Sony",
      "Canon",
      "HP",
      "Dell",
      "LG Electronics",
      "Philips",
      "Dyson",
      "Ford",
      "Hyundai",
      "Toyota",
      "Honda",
      "BMW",
      "Mercedes-Benz",
      "Ferrari",
      "Lamborghini",
      "Porsche",
      "Audi",
      "Volkswagen",
      "Chevrolet",
      "Nissan",
      "Jeep",
      "Rolls-Royce",
      "Harley-Davidson",
      "Yamaha",
      "Nike",
      "Adidas",
      "Puma",
      "Zara",
      "H&M",
      "Gucci",
      "Louis Vuitton",
      "Chanel",
      "Rolex",
      "Under Armour",
      "Levi's",
      "Calvin Klein",
      "Ray-Ban",
      "Vans",
      "Converse",
      "Prada",
      "Versace",
      "Lacoste",
      "Tommy Hilfiger",
      "Victoria's Secret",
      "Discovery Channel",
      "Cartoon Network",
      "Disney",
      "National Geographic",
      "Nickelodeon",
      "HBO",
      "BBC",
      "CNN",
      "MTV",
      "Warner Bros",
      "Marvel",
      "Animal Planet",
      "History Channel",
      "ESPN",
      "Coca-Cola",
      "Pepsi",
      "McDonald's",
      "Starbucks",
      "Red Bull",
      "KFC",
      "Pizza Hut",
      "Burger King",
      "Domino's Pizza",
      "Subway",
      "Nutella",
      "Oreo",
      "KitKat",
      "Nestl\u00e9",
      "Snickers",
      "Lay's",
      "Pringles",
      "Doritos",
      "Cadbury",
      "M&M's",
      "Lipton",
      "Nescaf\u00e9",
      "Ferrero Rocher",
      "Sprite",
      "Fanta",
      "7UP",
      "Monster Energy",
      "Mountain Dew",
      "Lego",
      "Ikea",
      "Gillette",
      "Dove",
      "Colgate",
      "Nivea",
      "Durex",
      "Pampers",
      "Head & Shoulders",
      "Vaseline",
      "Dettol",
      "Savlon (ACI)",
      "Meril",
      "Tibbet Snow",
      "Sandalina Soap",
      "Parachute Coconut Oil",
      "Square Pharmaceuticals",
      "Beximco Pharma",
      "Bashundhara Group",
      "RFL Plastics",
      "Gazi Group",
      "Walton",
      "Singer",
      "Vision Electronics",
      "Marcel",
      "Minister",
      "Symphony Mobile",
      "Grameenphone (GP)",
      "Banglalink",
      "Robi",
      "Teletalk",
      "bKash",
      "Nagad",
      "Pathao",
      "Foodpanda",
      "Daraz",
      "Chaldal",
      "Shwapno",
      "Aarong",
      "Pran",
      "Radhuni",
      "Fresh (Meghna Group)",
      "Teer (City Group)",
      "Akij",
      "Ispahani Mirzapore Tea",
      "Milk Vita",
      "Aarong Dairy",
      "Mojo",
      "Frutika",
      "Olympic Biscuits",
      "Dan Cake",
      "Igloo Ice Cream",
      "Polar Ice Cream",
      "Gloria Jean's Coffees",
      "North End Coffee",
      "Apex Footwear",
      "Bata",
      "Lotto",
      "Bay Emporium",
      "Yellow (Beximco)",
      "Sailor",
      "Cats Eye",
      "Ecstasy",
      "Richman",
      "Le Reve",
      "Gentle Park",
      "Taaga",
      "Kay Kraft",
      "Runner Automobiles",
      "Green Line Paribahan",
      "Hanif Enterprise",
      "Shyamoli Paribahan",
      "Shohoz",
      "Unimart",
      "Meena Bazar",
      "Agora"
    ]
  },
  {
    id: "movies-tv",
    name: "Movies & Series",
    emoji: "\ud83c\udfac",
    color: "#E8453C",
    description: "Blockbuster English, Bollywood & Bangladeshi cinema and hit series",
    words: [
      "Beder Meye Josna",
      "Hawa",
      "Toofan",
      "Aynabaji",
      "Monpura",
      "Dipu Number Two",
      "Debi",
      "Dhaka Attack",
      "Poran",
      "Shurongo",
      "Jibon Theke Neya",
      "Kabhi Khushi Kabhie Gham",
      "Keyamot Theke Keyamot",
      "Den Mohor",
      "Lal Shalu",
      "Dilwale Dulhania Le Jayenge",
      "Sholay",
      "3 Idiots",
      "Kabhi Khushi Kabhie Gham",
      "Dangal",
      "Kuch Kuch Hota Hai",
      "Lagaan",
      "Kal Ho Naa Ho",
      "Zindagi Na Milegi Dobara",
      "Taare Zameen Par",
      "PK",
      "Kabhi Khushi Kabhie Gham",
      "Chennai Express",
      "Om Shanti Om",
      "KGF",
      "Baahubali",
      "Chak De! India",
      "Don (Shah Rukh Khan)",
      "Krrish",
      "Dhoom",
      "Jab We Met",
      "My Name is Khan",
      "Munna Bhai M.B.B.S.",
      "Lage Raho Munna Bhai",
      "Hera Pheri",
      "Phir Hera Pheri",
      "Golmaal",
      "Welcome",
      "Yeh Jawaani Hai Deewani",
      "Stree",
      "Housefull",
      "Jawan",
      "Pathaan",
      "Animal",
      "12th Fail",
      "Student of the year",
      "Kabir Singh",
      "Sacred Games",
      "Mirzapur",
      "Scam 1992: The Harshad Mehta Story",
      "Harry Potter",
      "Titanic",
      "The Avengers",
      "Inception",
      "The Dark Knight",
      "Interstellar",
      "Avatar",
      "The Lord of the Rings",
      "Jurassic Park",
      "Star Wars",
      "The Matrix",
      "Shin-chan",
      "Oppenheimer",
      "Barbie",
      "Spider-Man",
      "Doremon",
      "The Lion King",
      "Forrest Gump",
      "Superman",
      "Pirates of the Caribbean",
      "Batman",
      "Home Alone",
      "Iron Man",
      "Pokemon",
      "The Prestige",
      "Fast & Furious",
      "John Wick",
      "Dune",
      "Deadpool",
      "Breaking Bad",
      "Game of Thrones",
      "Squid Game",
      "Money Heist",
      "Friends",
      "The Office",
      "Sherlock",
      "The Dark Night Rises"
    ]
  },
  {
    id: "act-it-out",
    name: "Act It Out",
    emoji: "\ud83c\udfad",
    color: "#8B5CF6",
    description: "No talking allowed! Act out funny actions and scenarios",
    words: [
      "Brushing your teeth",
      "Walking on a rope",
      "Opening a bottle",
      "Stuck in heavy traffic",
      "Surfing a huge wave",
      "Walking on the Moon",
      "Riding a crazy rollercoaster",
      "Sneezing",
      "Eating a pizza slice",
      "Eating extremely spicy chili",
      "Milking a cow",
      "Playing a guitar",
      "Solving a puzzle",
      "Skiing down a steep mountain",
      "Getting caught in torrential rain",
      "Taking a flattering selfie",
      "Riding a bike",
      "Running away from a bee",
      "Singing loudly in the shower",
      "Zombie walking",
      "Juggling flaming torches",
      "Sumo wrestling match",
      "Changing a baby diaper",
      "Walking against strong wind",
      "Dancing ballet",
      "Winning the lottery jackpot",
      "Opening a jar",
      "Karate kicking",
      "Escaping quicksand",
      "Bus conductor",
      "Tasting sour lemon",
      "Arm wrestling",
      "Walking on hot weather",
      "Bowling a strike",
      "Hypnotizing someone",
      "Putting on makeup",
      "Crying",
      "Looking for someone",
      "Figting with someone",
      "Swimming in a pool",
      "Taking a Bath",
      "Smoking",
      "Sleeping"
    ]
  },
  {
    id: "animals-nature",
    name: "Animals & Wildlife",
    emoji: "\ud83e\udd81",
    color: "#1E9E6A",
    description: "From deep sea creatures to jungle predators",
    words: [
      "Lion",
      "Kangaroo",
      "Penguin",
      "African Elephant",
      "Chimpanzee",
      "Bottlenose Dolphin",
      "Giraffe",
      "Sloth",
      "Rat",
      "Giant Octopus",
      "Cheetah",
      "Giant Panda",
      "Koala",
      "Peacock",
      "Crocodile",
      "Silverback Gorilla",
      "Zebra",
      "Great Horned Owl",
      "Human",
      "Seahorse",
      "Polar Bear",
      "Platypus",
      "Buffalo ",
      "Shark",
      "Bald Eagle",
      "Parrot",
      "Turtle",
      "Cuckoos",
      "Wolf",
      "Hippopotamus",
      "Komodo Dragon",
      "Hummingbird",
      "Snake",
      "Grizzly Bear",
      "Ostrich",
      "Blue Whale",
      "Jellyfish",
      "Red Fox",
      "Monkey",
      "Tiger",
      "Crow",
      "Camel",
      "Squid",
      "Orca",
      "Mongoose",
      "Duck",
      "Goose"
    ]
  },
  {
    id: "celebrities",
    name: "Superstars & Icons",
    emoji: "\ud83c\udf1f",
    color: "#D6336C",
    description: "Legends, icons, and superstars from Asia and across the globe",
    words: [
      "Shakib Al Hasan",
      "Mashrafe Bin Mortaza",
      "Tamim Iqbal",
      "Mushfiqur Rahim",
      "Mustafizur Rahman",
      "Humayun Ahmed",
      "James (Nagar Baul)",
      "Ayub Bachchu",
      "Runa Laila",
      "Sabina Yasmin",
      "Chanchal Chowdhury",
      "Mosharraf Karim",
      "Shakib Khan",
      "Salman Shah",
      "Jaya Ahsan",
      "Tahsan Khan",
      "Asif Akbar",
      "Habibul Bashar",
      "Sachin Tendulkar",
      "Virat Kohli",
      "MS Dhoni",
      "Rohit Sharma",
      "Shah Rukh Khan",
      "Amitabh Bachchan",
      "Salman Khan",
      "Aamir Khan",
      "Arijit Singh",
      "Lata Mangeshkar",
      "A.R. Rahman",
      "Shreya Ghoshal",
      "Kishore Kumar",
      "Deepika Padukone",
      "Priyanka Chopra",
      "Rajinikanth",
      "Hrithik Roshan",
      "Kapil Dev",
      "Sourav Ganguly",
      "Sania Mirza",
      "Alia Bhatt",
      "Shahid Afridi",
      "Atif Aslam",
      "Babar Azam",
      "Wasim Akram",
      "Imran Khan",
      "Shoaib Akhtar",
      "Rahat Fateh Ali Khan",
      "Cristiano Ronaldo",
      "Lionel Messi",
      "Taylor Swift",
      "Michael Jackson",
      "Tom Cruise",
      "Leonardo DiCaprio",
      "Dwayne \"The Rock\" Johnson",
      "Elon Musk",
      "MrBeast",
      "Jackie Chan",
      "Eminem",
      "Justin Bieber",
      "Selena Gomez",
      "Beyonc\u00e9",
      "Rihanna",
      "Keanu Reeves",
      "Brad Pitt",
      "David Beckham",
      "Gordon Ramsay",
      "Morgan Freeman",
      "Adele",
      "Ed Sheeran",
      "Bruno Mars",
      "Billie Eilish",
      "Michael jackson",
      "John Cena",
      "Undertaker",
      "Donald Trump",
      "Barack Obama",
      "William Shakespeare",
      "Albert Einstein",
      "Isaac Newton",
      "Charlie Chaplin",
      "Adolf Hitler",
      "Muhammad Ali"
    ]
  },
  {
    id: "food-drinks",
    name: "Food & Drinks",
    emoji: "\ud83c\udf55",
    color: "#F0803C",
    description: "Mouth-watering dishes, snacks, and beverages",
    words: [
      "Pizza Margherita",
      "Sushi & Sashimi",
      "Chocolate Ice Cream",
      "Crispy Tacos",
      "French Fries",
      "Pancakes with Maple Syrup",
      "Boba Bubble Tea",
      "Double Cheeseburger",
      "Hot Espresso",
      "Spaghetti Bolognese",
      "Guacamole and Tortilla Chips",
      "Fluffy Cotton Candy",
      "Butter Croissant",
      "Japanese Ramen",
      "Chicago Hot Dog",
      "Sweet Watermelon",
      "Spicy Burrito",
      "Glazed Donuts",
      "Avocado Toast",
      "Barbecue Ribs",
      "Buttery Popcorn",
      "Cinnamon Churros",
      "Belgian Waffles",
      "Chicken Nuggets",
      "Lasagna",
      "Fish and Chips",
      "Chocolate Milkshake",
      "Pad Thai",
      "Fresh Dim Sum",
      "Cheese Fondue",
      "Biryani",
      "Falafel",
      "Samosa",
      "Tiramisu",
      "Smoothie Bowl"
    ]
  },
  {
    id: "world-travel",
    name: "World Landmarks",
    emoji: "\ud83c\udf0d",
    color: "#0E9BA6",
    description: "Famous monuments, natural wonders, and great cities",
    words: [
      "Eiffel Tower (Paris)",
      "Statue of Liberty (New York)",
      "Great Wall of China",
      "Taj Mahal (India)",
      "Great Pyramids of Giza",
      "Colosseum (Rome)",
      "London",
      "Mount Everest",
      "Cox's Bazar",
      "Sundarban",
      "Mount Fuji (Japan)",
      "Sreemongol",
      "Amazon River",
      "Neil River",
      "Burj Khalifa (Dubai)",
      "Sahara Desert",
      "Kashmir",
      "Panjab",
      "Hollywood Sign (Los Angeles)",
      "Petra (Jordan)",
      "Bandarban",
      "Bagdad",
      "Tehran",
      "Mecca",
      "Madina",
      "Riyad"
    ]
  },
  {
    id: "brands",
    name: "Brands",
    emoji: "\ud83d\udecd\ufe0f",
    color: "#2563EB",
    description: "Famous global & popular brands \u2014 tech, cars, fashion, media, food & retail",
    words: [
      "Google",
      "YouTube",
      "Facebook (Meta)",
      "Instagram",
      "TikTok",
      "WhatsApp",
      "Alibaba",
      "Amazon",
      "Apple (iPhone)",
      "Microsoft",
      "Netflix",
      "Spotify",
      "Twitter (X)",
      "ChatGPT (OpenAI)",
      "Uber",
      "Airbnb",
      "LinkedIn",
      "Reddit",
      "PlayStation (Sony)",
      "Xbox",
      "Nintendo",
      "Intel",
      "Nvidia",
      "Tesla",
      "Samsung",
      "Xiaomi",
      "Sony",
      "Canon",
      "HP",
      "Dell",
      "LG Electronics",
      "Philips",
      "Dyson",
      "Ford",
      "Hyundai",
      "Toyota",
      "Honda",
      "BMW",
      "Mercedes-Benz",
      "Ferrari",
      "Lamborghini",
      "Porsche",
      "Audi",
      "Volkswagen",
      "Chevrolet",
      "Nissan",
      "Jeep",
      "Rolls-Royce",
      "Harley-Davidson",
      "Yamaha",
      "Nike",
      "Adidas",
      "Puma",
      "Zara",
      "H&M",
      "Gucci",
      "Louis Vuitton",
      "Chanel",
      "Rolex",
      "Under Armour",
      "Levi's",
      "Calvin Klein",
      "Ray-Ban",
      "Vans",
      "Converse",
      "Prada",
      "Versace",
      "Lacoste",
      "Tommy Hilfiger",
      "Victoria's Secret",
      "Discovery Channel",
      "Cartoon Network",
      "Disney",
      "National Geographic",
      "Nickelodeon",
      "HBO",
      "BBC",
      "CNN",
      "MTV",
      "Warner Bros",
      "Marvel",
      "Animal Planet",
      "History Channel",
      "ESPN",
      "Coca-Cola",
      "Pepsi",
      "McDonald's",
      "Starbucks",
      "Red Bull",
      "KFC",
      "Pizza Hut",
      "Burger King",
      "Domino's Pizza",
      "Subway",
      "Nutella",
      "Oreo",
      "KitKat",
      "Nestl\u00e9",
      "Snickers",
      "Lay's",
      "Pringles",
      "Doritos",
      "Cadbury",
      "M&M's",
      "Lipton",
      "Nescaf\u00e9",
      "Ferrero Rocher",
      "Sprite",
      "Fanta",
      "7UP",
      "Monster Energy",
      "Mountain Dew",
      "Lego",
      "Ikea",
      "Gillette",
      "Dove",
      "Colgate",
      "Nivea",
      "Durex",
      "Pampers",
      "Head & Shoulders",
      "Vaseline",
      "Dettol",
      "Savlon (ACI)",
      "Meril",
      "Tibbet Snow",
      "Sandalina Soap",
      "Parachute Coconut Oil",
      "Square Pharmaceuticals",
      "Beximco Pharma",
      "Bashundhara Group",
      "RFL Plastics",
      "Gazi Group",
      "Walton",
      "Singer",
      "Vision Electronics",
      "Marcel",
      "Minister",
      "Symphony Mobile",
      "Grameenphone (GP)",
      "Banglalink",
      "Robi",
      "Teletalk",
      "bKash",
      "Nagad",
      "Pathao",
      "Foodpanda",
      "Daraz",
      "Chaldal",
      "Shwapno",
      "Aarong",
      "Pran",
      "Radhuni",
      "Fresh (Meghna Group)",
      "Teer (City Group)",
      "Akij",
      "Ispahani Mirzapore Tea",
      "Milk Vita",
      "Aarong Dairy",
      "Mojo",
      "Frutika",
      "Olympic Biscuits",
      "Dan Cake",
      "Igloo Ice Cream",
      "Polar Ice Cream",
      "Gloria Jean's Coffees",
      "North End Coffee",
      "Apex Footwear",
      "Bata",
      "Lotto",
      "Bay Emporium",
      "Yellow (Beximco)",
      "Sailor",
      "Cats Eye",
      "Ecstasy",
      "Richman",
      "Le Reve",
      "Gentle Park",
      "Taaga",
      "Kay Kraft",
      "Runner Automobiles",
      "Green Line Paribahan",
      "Hanif Enterprise",
      "Shyamoli Paribahan",
      "Shohoz",
      "Unimart",
      "Meena Bazar",
      "Agora"
    ]
  }
]
